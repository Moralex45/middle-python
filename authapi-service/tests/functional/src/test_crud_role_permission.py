...
def test_scrap_existing_roles_permissions(flask_test_client,
                                          ...
                                          role_permission):
    response = flask_test_client.get(f'/api/v1/crud/role_permission/{role_permission["id"]}')
...
def test_scrap_non_existing_roles_permissions_filtered_by_role_id(flask_test_client,
                                                                  clean_database,
                                                                  generate_roles_permissions):
    response = flask_test_client.get(f'/api/v1/crud/role_permission/?role_id={uuid.uuid4()}')
    assert response.status_code == HTTPStatus.OK
    assert response.is_json
...
def test_scrap_roles_permissions_filtered_by_inappropriate_role_id_format(flask_test_client,
                                                                          clean_database,
                                                                          generate_roles_permissions):
    response = flask_test_client.get(f'/api/v1/crud/role_permission/?role_id={str(uuid.uuid4())[-1:]}')
...
def test_delete_existing_roles_permissions(flask_test_client, clean_database, generate_roles_permissions):
    ...
    response = flask_test_client.delete(f'/api/v1/crud/role_permission/{role_permission["id"]}')
    assert response.status_code == HTTPStatus.OK
    assert response.text == ''
...
def test_create_role_permission(flask_test_client, clean_database, generate_roles, generate_permissions):
    ...
    response = flask_test_client.post('/api/v1/crud/role_permission/', json=request_body)
    assert response.status_code == HTTPStatus.OK
    assert response.is_json

    created_role_permission_id = response.json['id']
    response = flask_test_client.get(f'/api/v1/crud/role_permission/{created_role_permission_id}')
    assert response.status_code == HTTPStatus.OK
    assert response.is_json
